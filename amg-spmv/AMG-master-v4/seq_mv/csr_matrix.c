...
#ifdef SPMV_OPT
   matrix->csrHandle=NULL;
   matrix->csrHandleT=NULL;
#endif
...
#ifdef SPMV_OPT
   if (matrix->csrHandle) destroyCSRHandle(matrix->csrHandle);
   if (matrix->csrHandleT) destroyCSRHandle(matrix->csrHandleT);
#endif
