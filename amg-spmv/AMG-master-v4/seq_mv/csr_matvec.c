...
#include "seq_mv.h"
#ifdef MKL
/* mkl_spblas.h and mkl_service enter through seq_mv.h */
#define SUCCESSFUL_RETURN SPARSE_STATUS_SUCCESS
#endif
#ifdef ARMPL
/* armpl.h enters through seq_mv.h */
...
#ifdef SPMV_OPT
     if (A->csrHandle) { // Handle needs to have been created...  
       if ((num_cols != 0) && (alpha !=0.0)) {
	 /*
	  * Note hypre_CSRMatrixGetLoadBalancedPartitionBegin(A) partitions
	  * matrix A so that it is load balanced over threads. It is assumed
	  * that it will be called from within a threaded region.
	  */
	 HYPRE_Int iBegin = 0;
	 HYPRE_Int iEnd = num_rows;
...
#pragma omp parallel for private(i) HYPRE_SMP_SCHEDULE
	 ...
	 status = mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE,
				  alpha, *(A->csrHandle), (A->csrMtxDescr),
				  x_data, beta, y_data );
#endif
#ifdef ARMPL
	 status = armpl_spmv_exec_d(ARMPL_SPARSE_OPERATION_NOTRANS,
				    alpha, *(A->csrHandle), x_data, beta, y_data );
#endif

       } else { /* matrix multiplication can be ignored...*/
	 HYPRE_Int iBegin = 0;
	 HYPRE_Int iEnd = num_rows;
...
#ifdef SPMV_OPT
   if (A->csrHandleT) { // Handle needs to have been created...  
     interface_rtn_status_t status = SUCCESSFUL_RETURN;

#ifdef MKL
     status = mkl_sparse_d_mv(SPARSE_OPERATION_TRANSPOSE,
			      alpha, *(A->csrHandleT), (A->csrMtxDescr),
			      x_data, beta, y_data );
#endif
#ifdef ARMPL
     status = armpl_spmv_exec_d(ARMPL_SPARSE_OPERATION_TRANS,
				alpha, *(A->csrHandleT), x_data, beta, y_data );
#endif
   } else {// no handle, use old way...
#endif
