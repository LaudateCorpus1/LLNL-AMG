                                    ...
                                    hypre_ParVector    *y )
{
   ...
   int                doInterpolatorP=0;	
   int 		      doSmootherA=0;
   #ifdef HPE_NBC_COMM
	   if(comm_pkg) {
	           doSmootherA=hypre_ParCSRCommPkgDoSmoother(comm_pkg);
	           doInterpolatorP=hypre_ParCSRCommPkgDoInterpolator(comm_pkg);
	   }
   #endif
   /*---------------------------------------------------------------------
...
#pragma omp parallel for HYPRE_SMP_SCHEDULE
   ...
   */
   ...
   else
   {
      ...
      {
	#ifdef HPE_NBC_COMM
	if (comm_pkg && (doSmootherA || doInterpolatorP))
	  {
		 //printf("A=%d, P=%d\n", doSmootherA,doInterpolatorP);
         comm_handle[jv] = hypre_ParCSRCommHandleCreateNBC
   ...
   else
   {
	 ...
	 if (comm_pkg && (doSmootherA || doInterpolatorP) )
                {
	         hypre_ParCSRCommHandleDestroyNBC(comm_handle[jv]);
//		printf("A=%d, P=%d\n", doSmootherA,doInterpolatorP);
	}
	else
                           ...
                           hypre_ParVector    *y )
{
   ...
   #ifdef HPE_NBC_COMM
	   if (comm_pkg){
           doRestrictorR=hypre_ParCSRCommPkgDoRestrictor(comm_pkg);}
   #endif
