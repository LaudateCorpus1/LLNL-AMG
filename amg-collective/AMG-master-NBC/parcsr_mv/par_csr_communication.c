...
hypre_ParCSRCommHandle *
hypre_ParCSRCommHandleCreateNBC ( HYPRE_Int            job,
                               hypre_ParCSRCommPkg *comm_pkg,
                               void                *send_data,
                               void                *recv_data )
{
   ...
   HYPRE_Int                  num_recvs = hypre_ParCSRCommPkgNumRecvs(comm_pkg);
   MPI_Comm                   comm      = hypre_ParCSRCommPkgComm(comm_pkg);
   MPI_Comm                   graph_comm = hypre_ParCSRCommPkgGraphComm(comm_pkg);

   hypre_ParCSRCommHandle    *comm_handle;
   HYPRE_Int                  num_requests = 0;
   ...
   HYPRE_Int                  my_id, num_procs;
   HYPRE_Int                  ip, vec_start, vec_len;
   unsigned int                 *sendcounts,*recvcounts,*rdispls,*sdispls;
   sendcounts = recvcounts = rdispls = sdispls = NULL;
   ...
   j = 0;
         ...
         void *send_data_startaddr=NULL;
         void *recv_data_startaddr=NULL;
   ...
   switch (job)
   {
      ...
      {
        if (num_recvs) {
            recvcounts=hypre_CTAlloc(int, num_recvs);
            rdispls=hypre_CTAlloc(int, num_recvs);
            rdispls[0]=0;
            vec_start = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,0);
            recv_data_startaddr = &((HYPRE_Complex *) recv_data)[vec_start];
            recvcounts[0] = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,1)-vec_start;
        }

        if (num_sends) {
            sendcounts=hypre_CTAlloc(int, num_sends);
            sdispls=hypre_CTAlloc(int, num_sends);
            sdispls[0]=0;
            vec_start = hypre_ParCSRCommPkgSendMapStart(comm_pkg, 0);
            send_data_startaddr = &((HYPRE_Complex *) send_data)[vec_start];
            sendcounts[0] = hypre_ParCSRCommPkgSendMapStart(comm_pkg, 1)-vec_start;
        }

         send_type = recv_type = HYPRE_MPI_COMPLEX;

         for (i = 1; i < num_recvs; i++)
         {
            vec_start = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,i);
            recvcounts[i] = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,i+1)-vec_start;
            rdispls[i]=rdispls[i-1]+recvcounts[i-1];
         }
         for (i = 1; i < num_sends; i++)
         {
            vec_start = hypre_ParCSRCommPkgSendMapStart(comm_pkg, i);
	    sendcounts[i] = hypre_ParCSRCommPkgSendMapStart(comm_pkg, i+1)-vec_start;
      ...
      {
        if (num_sends) {
         recvcounts=hypre_CTAlloc(int, num_sends);
         rdispls=hypre_CTAlloc(int, num_sends);
         rdispls[0]=0;
         vec_start = hypre_ParCSRCommPkgSendMapStart(comm_pkg, 0);
         recv_data_startaddr = &((HYPRE_Complex *) recv_data)[vec_start];
         recvcounts[0] = hypre_ParCSRCommPkgSendMapStart(comm_pkg,1)-vec_start;
        }

        if (num_recvs) {
         sendcounts=hypre_CTAlloc(int, num_recvs);
         sdispls=hypre_CTAlloc(int, num_recvs);
         sdispls[0]=0;
         vec_start = hypre_ParCSRCommPkgRecvVecStart(comm_pkg, 0);
         send_data_startaddr = &((HYPRE_Complex *) send_data)[vec_start];
         sendcounts[0] = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,1)-vec_start;
        }

         send_type = recv_type = HYPRE_MPI_COMPLEX;
         graph_comm = hypre_ParCSRCommPkgGraphCommT(comm_pkg);

         for (i = 1; i < num_sends; i++)
         {
            vec_start = hypre_ParCSRCommPkgSendMapStart(comm_pkg, i);
            recvcounts[i] = hypre_ParCSRCommPkgSendMapStart(comm_pkg,i+1)-vec_start;
            rdispls[i] = rdispls[i-1] + recvcounts[i-1];
         }
         for (i = 1; i < num_recvs; i++)
         {
            vec_start = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,i);
            sendcounts[i] = hypre_ParCSRCommPkgRecvVecStart(comm_pkg,i+1)-vec_start;
            sdispls[i] = sdispls[i-1] + sendcounts[i-1];
	 }
         break;
      }
   ...
   if(num_sends || num_recvs) {
       num_requests = 1;
       requests = hypre_CTAlloc(hypre_MPI_Request, num_requests);
       MPI_Ineighbor_alltoallv(send_data_startaddr, sendcounts, sdispls, send_type,
                recv_data_startaddr, recvcounts, rdispls, recv_type,
                graph_comm, &requests[j]);
   }


         if (sendcounts) hypre_TFree(sendcounts);
         if (sdispls) hypre_TFree(sdispls);
         if (recvcounts) hypre_TFree(recvcounts);
         if (rdispls) hypre_TFree(rdispls);
...
HYPRE_Int
hypre_ParCSRCommHandleDestroyNBC( hypre_ParCSRCommHandle *comm_handle )
{
   ...
   if (hypre_ParCSRCommHandleNumRequests(comm_handle))
   {
      status0 = hypre_CTAlloc(hypre_MPI_Status,
                              hypre_ParCSRCommHandleNumRequests(comm_handle));
        hypre_MPI_Wait(hypre_ParCSRCommHandleRequests(comm_handle), status0);
